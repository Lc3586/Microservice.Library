...
namespace Integrate_Business
{
    ...
    public class BaseBusiness<T> : IBaseBusiness<T>, IDependency where T : class, new()
    {
            ...
            get
            {
                if (_service == null) //双if +lock
                {
                    lock (_serviceLock)
                    {
                        if (_service == null)
                        {
                            _service = DbFactory.GetRepository(_conString, _entityAssembly, _dbType);
        ...
        {
            return new BusRepository(DbFactory.GetRepository(_conString, _entityAssembly, _dbType));
        }
        ...
        {
            return new BusRepository(DbFactory.GetRepository(conString, _entityAssembly, dbType));
        }
