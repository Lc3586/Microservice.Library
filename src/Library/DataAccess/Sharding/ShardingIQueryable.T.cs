    ...
    public class ShardingQueryable<T> : IShardingQueryable<T> where T : class, new()
    {
        ...
        public List<T> ToList()
        {
            ...
            tables.ForEach(aTable =>
            {
                tasks.Add(Task.Run(() =>
                {
                    ...
                    var targetDb = DbFactory.GetRepository(aTable.conString, null, aTable.dbType);
                    if (_openTransaction)
        ...
        private List<dynamic> GetStatisData(Func<IQueryable, dynamic> access, IQueryable<T> newSource = null)
        {
            ...
            tables.ForEach(aTable =>
            {
                ...
                {
                    var targetTable = MapTable(_absTableType, aTable.tableName);
                    var targetIQ = DbFactory.GetRepository(aTable.conString, null, aTable.dbType).GetIQueryable(targetTable);
        ...
        {
            var list = GetStatisData(x => new KeyValuePair<int, dynamic>(x.Count(), IQueryableExtention.DynamicSum(x, selector))).Select(x => (KeyValuePair<int, dynamic>)x).ToList();
        ...
        {
            return GetStatisData(x => IQueryableExtention.DynamicSum(x, selector)).Sum(x => (decimal?)x);
        }
