...
namespace Library.Http
{
    ...
    public static class HttpHelper
    {
        ...
        public static (HttpStatusCode, string) RequestData(string method, string url, string body, string contentType, Dictionary<string, string> headers = null, X509Certificate cerFile = null)
        {
            ...
            try
            {
                using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                {
                    ...
                    //    return (httpStatusCode, resData);
                    //}
                    return (response.StatusCode, GetResponseData(response));
                }
            }
        ...
        {
            byte[] resultByte = GetByte(response);
            string resData = null;
            if (resultByte != null && resultByte.Length > 0)
            {
                resData = SetEncoding(response, resultByte).GetString(resultByte);
            }
            else
            {
        ...
        private static byte[] GetByte(HttpWebResponse response)
        {
            ...
            {
                string contentEncoding = (response.ContentEncoding ?? string.Empty).ToLower();
                if (contentEncoding.Contains("gzip"))
                {
                    new GZipStream(response.GetResponseStream(), CompressionMode.Decompress).CopyTo(stream);
                }
                else if (contentEncoding.Contains("deflate"))
                {
                    new DeflateStream(response.GetResponseStream(), CompressionMode.Decompress).CopyTo(stream);
                }
                else
                {
        ...
        private static Encoding SetEncoding(HttpWebResponse response, byte[] responseByte)
        {
            ...
            if (str.Length > 2)
            {
                try
                {
                    return Encoding.GetEncoding(str.Replace("\"", string.Empty).Replace("'", "").Replace(";", "").Replace("iso-8859-1", "gbk").Trim());
                }
                catch
                {
                ...
                case ContentType.Form:
                    {
                        ...
                        for (int i = 0; i < paramList.Count; i++)
                        {
                            ...
                            bodyBuilder.Append($@"{head}{HttpUtility.UrlEncode(key, Encoding.UTF8)}={HttpUtility.UrlEncode(value, Encoding.UTF8)}");
                        }
                    }; break;
